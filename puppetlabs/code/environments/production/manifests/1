...
$repositorio = "/usr/bin/curl -fsSL https://download.docker.com/linux/debian/gpg | /usr/bin/apt-key add - ; /usr/bin/add-apt-repository 'deb [arch=amd64] https://download.docker.com/linux/ubuntu xenial stable' ; /usr/bin/apt update"
		}
	}
package{$pacotes:
	ensure => present,
